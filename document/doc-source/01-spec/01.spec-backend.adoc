...
|image:i-yaml.svg[,20] 文件|init/database.yml|（主入口专用）Liquibase专用生成数据表配置文件。
...
|image:i-folder.png[,20] image:i-java.svg[,20] 目录|/src/main/|（Maven）Java代码文件。
|image:i-folder.png[,20] image:i-config.svg[,20] 目录|/src/resources/|（Maven）Java类路径、资源文件。
